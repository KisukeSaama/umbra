...
| `POST /api/admin/search-sites`         | `{ name, url, queryParam, queryTemplate, params[], position?, enabled? }` | Adds a search page the queues can open. `url` must be http or https, `params` is `{ key, value }[]`, and a name already taken is a 409                                                                                                                                                                       |
